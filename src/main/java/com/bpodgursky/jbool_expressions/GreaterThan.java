...
public class GreaterThan<K> extends Expression<K> {

    public static final String EXPR_TYPE = "greater than";
    private String cachedStringRepresentation = null;

    private final Expression<K> e;

    private GreaterThan(Expression<K> e) {
        this.e = e;
    }

    public static <K> GreaterThan<K> of(Expression<K> e) {
        return new GreaterThan<K>(e);
    }
    ...
    public Expression<K> apply(List<Rule<?, K>> rules, ExprOptions<K> options) {
        ...
        if(e != this.e){
            return options.getExprFactory().gt(e);
        }
    ...
    public Expression<K> sort(Comparator<Expression> comparator) {
        return GreaterThan.of(e.sort(comparator));
    }
